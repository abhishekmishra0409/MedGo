...
import { logoutOwner } from "../../features/Owner/OwnerSlice.js";
...
const menuItems = [
    { icon: <FiHome className="text-lg" />, label: "Clinic", path: "" },
...
const DashboardModern = () => {
    ...
    const isApproved = profile?.ownerProfile?.approvalStatus === "approved";
    ...
    const isActive = (path) => {
        if (!path) {
            return location.pathname === "/clinic" || location.pathname === "/clinic/";
        }
        return location.pathname.includes(`/clinic/${path}`);
    };
                ...
                <div className="flex h-full flex-col overflow-hidden px-4 py-4">
                    <div className="mb-5 flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-teal-700">Facility workspace</p>
                            <p className="truncate text-sm text-slate-500">Clinic & roster management</p>
                        </div>
                    </div>
                        ...
                        <div className="mb-5 rounded-3xl border border-slate-200 bg-slate-50 p-3">
                            <div className="flex min-w-0 items-center gap-3">
                                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-teal-100 text-teal-800">
                                    <span className="font-semibold">{profile.name ? profile.name.charAt(0).toUpperCase() : "C"}</span>
                                </div>
                                <div className="min-w-0">
                                    <p className="truncate font-medium text-slate-800">{profile.name || "Clinic owner"}</p>
                ...
                <div className="w-full p-4 md:p-6 xl:p-8">
                    ...
                    {profile?.ownerProfile?.approvalStatus === "rejected" ? (
                        ...
                        <div className="auth-alert auth-alert--info mb-4">
                            Your facility is being verified. You can set up clinic details and review your doctor roster now — your
                            clinic goes live for patients once approved.
                        </div>
                    ) : null}
                    <Outlet />
                </div>
